...
public class Main {
    ...
    public static void main(String[] args) throws IOException, ParseException, EPCompileException, EPDeployException, InterruptedException {
        ...
        String query = args.length > 0 ? args[0] : "Q0001";

        stream_file = "/epl_cases/" +
                      query +
                      ".csv";

        File queryFile = new File(Objects.requireNonNull(Main.class.getResource("/epl_cases/Q0001.epl")).getPath());
        String statementId = query + "_Original;" +
                             query + "_Decomp_Final";

        Configuration config = new Configuration();
        run(query, queryFile, statementId, config);

    }


    private static void run(String query, File queryFile, String statementId, Configuration config) throws IOException, ParseException, EPCompileException, EPDeployException, InterruptedException {
        ...
        for (EPStatement statement : deploy.getStatements()) {
            ...
            if (annotation.value().equals("DML")) {
                if (statementId.contains(statement.getName())) {
                    ...
                    } else statement.addListener(new FileLogListener("./out/" + query + "/"));
                }
            } else {
